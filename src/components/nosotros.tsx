    ...
    <div className="w-full flex flex-col items-center p-6 mt-6">
      ...
      <div className="flex flex-col gap-6 max-w-3xl w-full">
        {agentsData.map((agent, index) => (
          <article key={index} className="flex flex-col md:flex-row items-center bg-card text-card-foreground rounded-lg shadow p-4 gap-4 border border-border">
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left flex-1">
              ...
              <p className="text-muted-foreground mb-1 flex items-center">
                ...
                <a href={`mailto:${agent.email}`} className="hover:text-primary ml-1">
                  {agent.email}
                </a>
              </p>
            </div>

            <Link href="/contact" className="text-accent hover:text-primary text-2xl">
              <FaArrowRight />
            </Link>
          </article>
        ))}
